...
//go through each term in this taxonomy one at a time
    ...
    $contest_places = new WP_Query($args_2);
global $wp_query;
?>
<style type="text/css">
    .like{
...
<section class="konkurses bizov-container" id="primary">
    ...
    <div class="content-area" style="">
        <div class="header-concurs"><span id="head-title"><?php  $tax = $wp_query->get_queried_object();
                $tax_term = $tax->name;
...
//                    echo "$tax_term";?><!--</span></div>-->
      <section class="left-part">
          ...
          <div class="concurs-header">
              ...
              $rating = esc_url(get_template_directory_uri()).'/images/rating-ico.png';?>
...
if (function_exists('wp_get_terms_meta')) {
    $metaValue = wp_get_terms_meta($term_id, 'first-place', true);
}
?>
          ...
          <h5>Фото участников</h5>
          <ul class="contests-list row">
        <?php
        if ($custom_loop->have_posts()):
        while ($custom_loop->have_posts()) : $custom_loop->the_post(); ?>
                <li class="concurs-entry-thumbnail col-md-4" style="">
                    <div class="contest-image">
                    <?php $post_medium_img = $fr_obj->get_thumbnail(get_the_ID(), 'small');
                    $post_full_img = $fr_obj->get_thumbnail(get_the_ID(), 'full');?>
                    <img style="visibility: visible; " width="auto" height="auto" src="<?php echo esc_url($post_medium_img);?>"  data-src="<?php echo esc_url($post_full_img);?>" class="fr_lazyload" alt="<?php the_title_attribute();?>">
                    </div>
                    <div class="rating-contest">
                        <span>Текущая оценка</span>
<!--                        <img src="--><?php //echo $rating;?><!--" alt="" width="20px" height="20px">-->
                        <?php echo do_shortcode( '[wishlist_button show_count="yes" show_menu="no" icon_active="" icon_inactive="" icon_loading="" ]' ); ?>
                        <?php //echo do_shortcode( '[social_votes]');
//                        sv_show_vote();
                        the_content();
                        ?>
                    </div>
                    ...
                    <a class="concurs-comments_link" href="<?php comments_link(); ?>"><i class="far fa-pencil"></i> Комментарии</a>&nbsp;
                    <?php $comments = wp_count_comments($post->ID);
                    echo '<span>('.$comments->total_comments.')</span>';
        ?>
                    </div>
                </li>
            <?php
