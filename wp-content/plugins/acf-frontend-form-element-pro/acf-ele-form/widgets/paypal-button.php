<?php
namespace ACFFrontendForm\Module\Widgets;

use Elementor\Controls_Manager;
use Elementor\Widget_Base;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

	
class Paypal_Buttons_Widget extends \Elementor\Widget_Base {

    public function __construct($data = [], $args = null) {
		parent::__construct($data, $args);
		wp_register_script( 'paypal-button', plugins_url( '../assets/js/paypal-button.js', __FILE__ ), [ 'elementor-frontend' ], '1.0.8', true );
		wp_register_style( 'paypal-front', plugins_url( '../assets/css/paypal-front.css', __FILE__ ));
		add_action( 'elementor/preview/enqueue_styles', function() {
			wp_enqueue_style( 'paypal-editor', plugins_url( '../assets/css/paypal-editor.css', __FILE__ ));
		 });
     }

	 public function get_script_depends() {
		return [ 'paypal-button' ];
	 }

	 public function get_style_depends() {
		return [ 'paypal-front' ];
	 }
	public function get_name() {
		return 'paypal-button-widget';
	}
	public function get_title() {
		return __( 'Paypal Button (BETA)', 'acf-frontend-form-element' );
    }
    public function get_icon() {
        return 'fab fa-paypal frontend-icon';
    }
    public function get_categories() {
		return [ 'acfef-widgets' ];
	}
    public function get_keywords() {
		return [ 'paypal', 'pay', 'button' ];
	}

	protected function _register_controls() {

		$this->start_controls_section(
			 'section_paypal_button',
			[
				'label' => 'PayPal button',
				'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
		$this->add_control(
			'custom_clientid',
			[
				'label'        => __( 'Custom Client ID ', 'acf-frontend-form-element' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'description' => __( 'Use the default Client ID (in the PayPal settings page) or use a custom one here', 'acf-frontend-form-element' ),
				'label_on'     => __( 'Custom', 'acf-frontend-form-element' ),
				'label_off'    => __( 'Defauld', 'acf-frontend-form-element' ),
				'return_value' => 'yes',
				'default'      => '',
				'separator'    => 'before',
			]
		);
        $this->add_control(
			'client_id_paypal_button',
			[
				'label' => __( 'Custom PayPal Client ID', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::TEXT,
                'placeholder' => __( 'Your PayPal Client ID', 'acf-frontend-form-element' ),
                'description' => __( 'You can add the live key or sandbox Client ID', 'acf-frontend-form-element' ),
				'dynamic' => [
					'active' => true,
				],	
				'condition' => [
					'custom_clientid' => 'yes',
				]
              //  'frontend_available' => true,		
			]
        );
        $this->add_control(
			'paypal_sandbox',
			[
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => __( '<p>You need to create PayPal App to get the Client ID</p>', 'acf-frontend-form-element' ),
				'content_classes' => 'acf-fields-note',
			]
		);
        $this->add_control(
			'paypal_app',
			[
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => __( '<p><a target="_blank" href="https://developer.paypal.com/developer/applications/create">Click here</a> to create new PayPal App or to get your App Client ID.</p>', 'acf-frontend-form-element' ),
                'content_classes' => 'acf-fields-note',
                'separator' => 'after',
			]
		);
		$this->add_control(
			'price_paypal_button',
			[
				'label' => __( 'Amount To Charge', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::NUMBER,
				'min' => 1,
				'placeholder' => __( '20', 'acf-frontend-form-element' ),
				'default' => __( '20', 'acf-frontend-form-element' ),
				'required' => true,
				'description' => __( 'Must be equal to or greater than 50 cents in USD', 'acf-frontend-form-element' ),
				'dynamic' => [
					'active' => true,
                ],
             //   'frontend_available' => true,
    		]
		);		
		$this->add_control(
			'currency_paypal_button',
			[
				'label' => __( 'Currency', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::TEXT,
				'placeholder' => __( 'USD', 'acf-frontend-form-element' ),
				'default' => __( 'USD', 'acf-frontend-form-element' ),
				'required' => true,
				'dynamic' => [
					'active' => true,
                ],	
             //  'frontend_available' => true,		
			]
		);
		$this->add_control(
			'paypal_currencies_button',
			[
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => __( '<p><a target="_blank" href="https://developer.paypal.com/docs/api/reference/currency-codes/">Click here</a> to learn about paypal\'s supported currencies.</p>', 'acf-frontend-form-element' ),
				'content_classes' => 'acf-fields-note',
			]
		);
		$this->add_control(
			'description_paypal_button',
			[
				'label' => __( 'Purchase Description', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
				'separator' => 'after',
             //   'frontend_available' => true,			
			]
		);
		$this->add_control(
			'after_pay',
			[
				'label' => __( 'Action After Payment', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'options' => [
					'thank-you' =>  __('Thank you message', 'acf-frontend-form-element' ),
					'redirect' =>  __('Redirect', 'acf-frontend-form-element' )
				],
				'default' => 'thank-you',
			//	'frontend_available' => true,
			]
		);
		$this->add_control(
			'thank_you',
			[
				'label' => __( 'Thank You Message', 'acf-frontend-form-element' ),
				'default' => __('Thank You For Your Payment', 'acf-frontend-form-element'),
				'type' => \Elementor\Controls_Manager::TEXTAREA,
				'dynamic' => [
					'active' => true,
				],
				'condition' => [
					'after_pay' => 'thank-you',
				],
				'separator' => 'after',
             //   'frontend_available' => true,			
			]
		);
		$this->add_control(
			'redirect_url',
			[
				'label' => __( 'Redirect After Payment', 'acf-frontend-form-element' ),
				'type' => Controls_Manager::URL,
				'condition' =>[
					'after_pay' => 'redirect',
				],
				'dynamic' => [
					'active' => true,
				],
				'placeholder' => __( 'https://your-link.com', 'acf-frontend-form-element' ),
				'separator' => 'before',
			]
		);
		$this->add_control(
			'hide_card',
			[
				'label'        => __( 'Hide Credit Card Button ', 'acf-frontend-form-element' ),
				'type'         => \Elementor\Controls_Manager::SELECT,
				'default' => 'vertical',
				'options' => [
					'vertical' =>  __('Show', 'acf-frontend-form-element' ),
					'horizontal' =>  __('Hide', 'acf-frontend-form-element' ),
				],
				'separator'    => 'before',
				'frontend_available' => true,
			]
		);

		$this->end_controls_section();
		
		$this->start_controls_section(
			'section_paypal_styles',
		   [
			   'label' => 'PayPal button',
			   'tab' => Controls_Manager::TAB_STYLE,
		   ]
	   );

		$this->add_control(
			'button_color_button',
			[
				'label' => __( 'Paypal Button Color', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'edit_post',
				'options' => [
					'gold' =>  __('Gold', 'acf-frontend-form-element' ),
					'blue' =>  __('Blue', 'acf-frontend-form-element' ),
					'silver' =>  __('Silver', 'acf-frontend-form-element' ),
					'black' =>  __('Black', 'acf-frontend-form-element' )
				],
				'default' => 'gold',
			//	'frontend_available' => true,
			]
		);
		$this->add_control(
			'button_style_button',
			[
				'label' => __( 'Paypal Button Style', 'acf-frontend-form-element' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'edit_post',
				'options' => [
					'pill' => __('Pill', 'acf-frontend-form-element' ),
					'rect' =>  __('Rect', 'acf-frontend-form-element' )
				],
				'default' => 'pill',
			//	'frontend_available' => true,
			]
		);
		$this->add_responsive_control(
			'button_width_button',
			[
				'label' => __( 'Buttons Width', 'acf-frontend-form-element' ) . ' (%)',
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => [
					'size' => 50,
				],
				'range' => [
					'%' => [
						'min' => 0,
						'max' => 100,
					],
				],
				'selectors' => [
					'{{WRAPPER}} .paypal-button-acf-warp' => 'width: {{SIZE}}%;',
				],
			]
		);
		$this->add_responsive_control(
			'button_align_button',
			[
				'label' => __( 'Alignment', 'elementor' ),
				'type' => \Elementor\Controls_Manager::CHOOSE,
				'options' => [
					'left' => [
						'title' => __( 'Left', 'elementor' ),
						'icon' => 'fa fa-align-left',
					],
					'none' => [
						'title' => __( 'Center', 'elementor' ),
						'icon' => 'fa fa-align-center',
					],
					'right' => [
						'title' => __( 'Right', 'elementor' ),
						'icon' => 'fa fa-align-right',
					],
				],
				'default' => 'none',
				'selectors' => [
					'{{WRAPPER}} .paypal-button-acf-warp' => 'float: {{VALUE}};',
				],
			]
		);
		$this->end_controls_tab();
			
			
			$this->end_controls_section();
	
	}
	
	
	protected function render() {
		$settings = $this->get_settings_for_display();
		$corrency = $settings['currency_paypal_button'];
		
		if (! empty($settings['client_id_paypal_button'])){
			$client_ID = $settings['client_id_paypal_button'];
		}
		elseif ( isset( get_option( 'paypal_settings_option_name' )['acfef_paypal_live_mode'] ) ){
			$client_ID = get_option( 'paypal_settings_option_name' )[ 'live_acfef_paypal_publishable_key' ];  
		}
		else{
			$client_ID =  get_option( 'paypal_settings_option_name' )[ 'test_acfef_paypal_publishable_key' ];
		}


		$color = $settings['button_color_button'];	
		$style = $settings['button_style_button'];
		$price = $settings['price_paypal_button'];
		$description = $settings['description_paypal_button'];
		$thank_you = $settings['thank_you'];
		$redirect = $settings['redirect_url']['url'];
		$card = $settings['hide_card'];
?>
<div class="paypal-button-acf-warp" data-id='{"price_paypal_button":<?php echo $price?>,"description_paypal_button":"<?php echo $description;?>","button_color_button":"<?php echo $color ?>","button_style_button":"<?php echo $style; if(! empty($thank_you)){?>","thank_you":"<?php echo $thank_you;}?><?php if(! empty($redirect)){?>","redirect":"<?php echo $redirect;} ?>","style":"<?php echo $card?>"}'>
<?php

if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
	//wp_enqueue_style( 'paypal-editor', ACFEF_URL . 'acf-ele-form/assets/css/paypal-editor.css' );		
	?>
	<div id="paypal-button-container" style="margin: auto;padding: 0;display: block;">
	<div id="buttons-container" class="buttons-container"><div class="paypal-button-container paypal-button-layout-vertical paypal-button-shape-<?php echo $style?> paypal-button-number-multiple paypal-button-env-sandbox">
<div role="button" data-button="" data-funding-source="paypal" class="paypal-button paypal-button-number-0 paypal-button-layout-vertical paypal-button-shape-<?php echo $style?>  paypal-button-number-multiple paypal-button-env-sandbox paypal-button-color-<?php echo $color?>  paypal-button-text-color-black paypal-logo-color-blue paypal-button-clicked" tabindex="0">
<?php if(($color == 'black') || ($color == 'blue') ){?>
<div class="paypal-button-label-container"><span class="paypal-button-text" style="color:#fff;" optional=""><?php echo __( 'Pay With ' , 'acf-frontend-form-element' ) ?> </span><img src="data:image/svg+xml;base64," data-v-f48c589a="" alt="PP" class="paypal-logo paypal-logo-pp paypal-logo-color-white"><span class="paypal-button-space"> </span><img src="data:image/svg+xml;base64," data-v-f48c589a="" alt="PayPal" class="paypal-logo paypal-logo-paypal paypal-logo-color-white"><span class="paypal-button-text" optional="">&rlm;</span></div>
<?php }
else{?>
<div class="paypal-button-label-container"><span class="paypal-button-text" optional=""><?php echo __( 'Pay With ' , 'acf-frontend-form-element' ) ?></span><img src="data:image/svg+xml;base64," data-v-f48c589a="" alt="PP" class="paypal-logo paypal-logo-pp paypal-logo-color-blue"><span class="paypal-button-space"> </span><img src="data:image/svg+xml;base64," data-v-f48c589a="" alt="PayPal" class="paypal-logo paypal-logo-paypal paypal-logo-color-blue"><span class="paypal-button-text" optional="">&rlm;</span></div>
<?php } ?>
<div class="paypal-button-spinner"></div></div>
<?php if($card == 'vertical'){ ?>
<div role="button" data-button="" data-funding-source="card" class="paypal-button paypal-button-number-1 paypal-button-layout-vertical paypal-button-shape-<?php echo $style?>  paypal-button-number-multiple paypal-button-env-sandbox paypal-button-color-black paypal-button-text-color-white paypal-logo-color-white" tabindex="0"><div class="paypal-button-label-container"><span class="paypal-button-text" optional=""><?php echo __( 'Credit Card ' , 'acf-frontend-form-element' ) ?></span><span class="paypal-button-space"> </span><img src="data:image/svg+xml;base64," alt="" class="paypal-logo-card paypal-logo-card-"></div><div class="paypal-button-spinner"></div></div>
<?php } ?>
<div class="paypal-powered-by">
<?php echo __( 'Powered by ', 'acf-frontend-form-element' ) ?>
<img src="data:image/svg+xml;base64," alt="PayPal" class="paypal-logo paypal-logo-paypal paypal-logo-color-blue"></div><script nonce="">(function(){var n="*",o="[optional]";function t(n){return[].slice.call(n)}function e(n,o){return t((o=o||document).querySelectorAll(n)).filter((function(n){return"style"!==n.tagName.toLowerCase()}))}function l(n){return parseInt(n.getAttribute("optional")||0,10)}function r(n){return function(n){for(var o=0,t=0;t<n.length;t++)o+=n[t];return o}(n.map((function(n){return n.offsetWidth})))}var i=function(n){for(var o=[],t=0;t<n.length;t++){var e=n[t];-1===o.indexOf(e)&&o.push(e)}return o}(e(o).map((function(n){return n.parentElement})).filter(Boolean)).map((function(r){return{optionalParent:r,allChildren:e(n,r),optionalChildren:t(e(o,r)).sort((function(n,o){return l(n)-l(o)}))}}));function a(){for(var n=0;n<i.length;n++)for(var o=i[n],t=o.optionalChildren,e=o.optionalParent.offsetWidth,l=r(o.allChildren)-r(t),a=0;a<t.length;a++){var C=t[a];(l+=C.offsetWidth)>e?C.classList.add("hidden"):C.classList.remove("hidden")}}var C,s,c,f=function(n){var o=!1;return function(){o||(o=!0,n.apply(void 0,arguments))}}((C=function(){window.addEventListener("resize",a),document.body&&document.body.classList.add("dom-ready")},void 0===s&&(s=50),function(){for(var n=arguments.length,o=new Array(n),t=0;t<n;t++)o[t]=arguments[t];clearTimeout(c),c=setTimeout((function(){C.apply(void 0,o)}),s)})),u=function(){a(),f()};a(),document.addEventListener("DOMContentLoaded",u),window.addEventListener("load",u),window.addEventListener("resize",u)})()</script></div></div>
</div>
<?php	}?>
		
	<div id="success" class="hide  -success acf-sucess-message"><?php echo $thank_you ?></div>	
	<div id="paypal-button-container" style="margin: auto;padding: 0;"></div>
		<script src="https://www.paypal.com/sdk/js?client-id=<?php echo $client_ID?>&currency=<?php echo $corrency?>"></script>
	
		</div>
<?php
		
	}

}