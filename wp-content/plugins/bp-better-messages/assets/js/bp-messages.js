...
/* Premium Code Stripped by Freemius */
        ...
        var threads_list = jQuery('.bp-messages-wrap:not(.bp-better-messages-list,.bp-better-messages-mini) .scroll-wrapper.scrollbar-inner.threads-list-wrapper');
        if( threads_list.length > 0 ){
            if( threads_list.get(0).scrollHeight <= threads_list.innerHeight() ) {
                loadMoreThreads(threads_list, true);
            }
        }


        jQuery('.scrollbar-inner').on('scroll', function(event){
            ...
            $.post(
                ...
                , function (html) {
                    ...
                    if( html.trim() === '' ){
                        ...
                        if( loadUntilScroll && element.get(0).scrollHeight <= element.innerHeight() ) {
        ...
        function changeMaxHeight(){
            ...
            jQuery('.bp-messages-wrap .scroller').css( 'max-height', height );
    ...
    function scrollBottom(target) {
        ...
        list.closest('.scroller').scrollTop(parseInt(scroll) + 100);
    }
    ...
    function updateThreads(message) {
        ...
        if( exist.length > 0 ){
            ...
            $(".bp-messages-wrap .threads-list").prependTo(html);
        }
