...
if ( !class_exists( 'BP_Better_Messages_Hooks' ) ):
    ...
    class BP_Better_Messages_Hooks
    {
        ...
        public function max_height_css(){
            ...
            echo '<style type="text/css">body:not(.bp-messages-mobile) .bp-messages-wrap.bp-messages-wrap-main > .scroller,body:not(.bp-messages-mobile) .bp-messages-wrap.bp-messages-wrap-main > .scroller > .scroller{max-height:'. $max_height .'px!important;}.bp-messages-side-threads-wrapper{height:' . ($max_height + 70) .'px!important;}</style>';
        }
            ...
            if ( function_exists( 'buddyboss_theme_register_required_plugins' ) || class_exists('BuddyBoss_Theme') ) {
                if( count( $array['threads'] ) > 0 && isset( $array['total'] ) ) {
                    foreach ($array['threads'] as $i => $thread) {
                        if ( ! strtotime($thread->last_message_date) ) {
