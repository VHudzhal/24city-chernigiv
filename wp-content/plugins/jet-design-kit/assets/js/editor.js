(function( $ ) {
	...
	var JetDesignKitData = window.JetDesignKitData || {},
		JetThemeEditor,
		JetThemeViews;
	...
	JetThemeViews = {
		...
		init: function() {
			...
			self.LibraryTemplateItemView = Marionette.ItemView.extend( {
				...
				onCloneButtonClick: function() {
					...
					$.ajax({
						...
						dataType: 'json',
						data: {
							action:  'jet_design_kit_clone_template',
			...
			self.LibraryLayoutView = Marionette.LayoutView.extend( {
				...
				regions: JetDesignKitData.modalRegions,
	...
	JetThemeEditor = {
		...
		onPreviewLoaded: function() {
			...
			this.tabs       = JetDesignKitData.tabs;
			this.defaultTab = JetDesignKitData.defaultTab;
		...
		initMagicButton: function() {
			...
			var $addNewSection = window.elementor.$previewContents.find( '.elementor-add-new-section' ),
				addJetTemplate = '<button class="add-jet-template" type="button">' + JetDesignKitData.libraryButton + '</button>',
				$addJetTemplate;

			if ( $addNewSection.length && JetDesignKitData.libraryButton ) {
			...
			window.elementor.$previewContents.on(
				...
				function() {
					...
					if ( JetDesignKitData.libraryButton ) {
						setTimeout( function() {
