				...
				<div class="col-xs-12">
											...
											<div class="col-sm-8">
												...
												<div class="form-group">
													<div class="col-sm-12" ng-class="{'has-error' : form.signed.$invalid && (form.signed.$dirty || submitted)}">
														<div class="checkbox license"><label><input type="checkbox" name="signed" ng-model="model.pc_signed" ng-checked="model.pc_valid" ng-disabled="model.pc_valid" ng-required="true"> <?php _e('I confirm that my purchase code will be defined for a single domain name at first validation according to regular license terms. It will be valid only for domain name that I entered including subdomain names and localhost for testing purposes.', 'ol-scrapes'); ?></label></div>
					...
					<div class="sidebar">
						<div class="fixed">
							...
							<button type="submit" name="submit" class="btn btn-primary" ng-class="{'disabled' : form.$invalid || model.pc_valid}" ng-click="submit($event)"><i class="icon ion-arrow-right-c"></i><span><?php _e('Save', 'ol_imports'); ?></span></button>
						</div>
					</div>
				</div>
			</div>
